...
help() {
    ...
    echo 'an environment file to use with NATS.'
    echo
}
...
envcheck() {
    ...
    if [ ! -f "_env" ]; then
        echo '# Environment variables for NATS' > _env
        ...
        echo CONSUL=consul-nats.svc.${TRITON_ACCOUNT}.${TRITON_DC}.cns.joyent.com >> _env
        echo >> _env

        echo 'Edit the _env file with your desired NATS_* config'
    else
